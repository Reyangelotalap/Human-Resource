<?php
	...
	if(isset($_POST['update'])){
		...
		//connection
		$conn = new mysqli('localhost', 'root', '', 'human_resource 1');

		//get user details
		$sql = "SELECT * FROM regapplicant WHERE id = '".$_SESSION['regapplicant']."'";
		...
		if(password_verify($old, $row['password'])){
			...
			if($new == $retype){
				...
				$sql = "UPDATE regapplicant SET password = '$password' WHERE id = '".$_SESSION['regapplicant']."'";
				if($conn->query($sql)){
